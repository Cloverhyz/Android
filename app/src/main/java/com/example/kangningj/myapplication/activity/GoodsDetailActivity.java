...
public class GoodsDetailActivity extends Activity implements View.OnClickListener {
    ...
    private void initData() {
        //获取传递的数据
        mGoodBean = (PaperBookMd) getIntent().getSerializableExtra("goodbean");
        //设置数据
        tvGoodInfoName.setText(mGoodBean.getBookName());
        tvGoodInfoPrice.setText("￥" + mGoodBean.getBookPicPath());
        Glide.with(getApplicationContext()).load(Constant.BOOK_PIC_URL + mGoodBean.getBookPicPath()).into(ivGoodInfoImage);
        initWebView();
    }
