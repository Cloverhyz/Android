...
import com.example.kangningj.myapplication.view.NumberAddSubView;
...
public class MyCartRecycleViewAdapter extends RecyclerView.Adapter<MyCartRecycleViewAdapter.MyViewHolder> {
    private final List<PaperBookMd> carts;
    private final Context mContext;
    private final CheckBox checkboxAll;
    private final TextView tvShopcartTotal;
    private final CheckBox cbAll;

    public MyCartRecycleViewAdapter(Context context, List<PaperBookMd> all, CheckBox checkboxAll, TextView tvShopcartTotal, CheckBox cbAll) {
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(final int position) {
            ...
            mTvPriceGov.setText("￥" + mBean.getBookPrice());
            mTvDescGov.setText(mBean.getBookName());
            Glide.with(mContext).load(Constant.BOOK_PIC_URL + mBean.getPaperBookId()).into(mIvGov);
            mNumberAddSubView.setValue(mBean.getNumber()); //加入购物车数量
            mNumberAddSubView.setMaxValue(mBean.getNumber()); //库存
            mNumberAddSubView.setMinValue(1); //最小值
            mCbGov.setChecked(mBean.getCheckd()==1);
        ...
        private void initEvent(final int position) {
            ...
            setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(View view, int position) {
                    //状态取反
                    PaperBookMd good = carts.get(position);
                    good.setCheckd(good.getCheckd()==1?0:1);
    ...
    public double getTotalPrice() {
        ...
        for (int i = 0; i < carts.size(); i++) {
            ...
            if (bean.getCheckd()==1) {
                //当前选中的条目才去累加价格
                totalPrice = totalPrice + bean.getBookPrice() * bean.getNumber();
            }
        }
        return totalPrice;
    }
